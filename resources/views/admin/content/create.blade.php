					...
					<div class="col-lg-12">
						<div class="card mb-4 card-primary shadow">
							<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
								...
								<a href="{{ route('content.index', $content) }}" class="btn btn-sm btn-outline-primary"> <i class="fas fa-arrow-left"></i> @lang('Back')</a>
							</div>
							<div class="card-body">
								@if(array_key_exists('language',config("contents.$content")) && config("contents.$content.language") == 0)
									<form method="post" action="{{ route('content.store', [$content,0]) }}" enctype="multipart/form-data">
										@csrf
										@method('put')
										<div class="row">
									...
									<div class="tab-content mt-2" id="myTabContent">
										@foreach($languages as $key => $language)
											<div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-tab-{{ $key }}" role="tabpanel">
												<form method="post" action="{{ route('content.store', [$content,$language->id]) }}" enctype="multipart/form-data">
													@csrf
													@method('put')
													<div class="row">
														@foreach(config("contents.$content.field_name") as $name => $type)
